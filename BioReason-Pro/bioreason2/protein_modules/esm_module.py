from typing import Dict, Any, Union, List, Callable, Type
from trl.data_utils import maybe_apply_chat_template
import torch

from bioreason2.protein_modules.protein_module import ProteinBaseModule
from bioreason2.models.protein_llm import ProteinLLMModel
from bioreason2.models.pl.processing_pl import PLProcessor


class ESMProteinModule(ProteinBaseModule):
    """
    Protein module implementation for ESM3-based models.
    ...
    def __init__(self):
        """Initialize the ESMProteinModule."""
        super().__init__()
    ...
    def prepare_prompt(self, processing_class: Any, inputs: List[Dict[str, Union[torch.Tensor, Any]]]) -> List[str]:
        """
    ...
    def select_reward_func(func: str, task_type: str) -> Callable:
        ...
        if func == "accuracy":
            match task_type:
                case "rec":
                    return ESMProteinModule.iou_reward
                case _:
                    raise ValueError(f"Unsupported reward function: {func}")
        elif func == "format":
            match task_type:
                case "rec":
                    return ESMProteinModule.format_reward_rec
                case _:
                    raise ValueError(f"Unsupported reward function: {func}")
        else:
            raise ValueError(f"Unsupported reward function: {func}")
