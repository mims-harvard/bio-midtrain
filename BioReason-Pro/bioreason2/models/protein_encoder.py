...
class ESMCEncoder(ProteinEncoder):
    ...
    def _get_embedding_dim(self) -> int:
        ...
        # Prefer introspection from the loaded model, which is robust across ESM versions.
        if hasattr(self.model, "embed"):
            embed = getattr(self.model, "embed")
            if hasattr(embed, "embedding_dim"):
                return int(embed.embedding_dim)
            if hasattr(embed, "weight") and hasattr(embed.weight, "shape"):
                return int(embed.weight.shape[-1])

        # Fallback to known dimensions if model internals differ across versions.
        known_dims = {
            "esmc_300m": 960,
            "esmc_600m": 1152,
        }
