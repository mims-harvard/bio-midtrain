...
QWEN3_TRANSFORMERS_FALLBACK_MODEL = "Qwen/Qwen2.5-1.5B-Instruct"
ESM3_GATED_FALLBACK_MODEL = "esmc_300m"
    ...
    def __init__(
        ...
        if self.use_unsloth:
            try:
                ...
                print("⚠️  Unsloth requested in ProteinLLMModel but is unavailable. Using standard HuggingFace loading instead.")
        ...
        if self.use_unsloth:
            ...
            resolved_text_model_name = text_model_name
            try:
                self.text_model = AutoModelForCausalLM.from_pretrained(
                    resolved_text_model_name,
                    **text_model_kwargs
                )
                ...
                error_text = str(e)
                # Older transformers versions may not recognize Qwen3 yet.
                if "model type `qwen3`" in error_text:
                    fallback_model = QWEN3_TRANSFORMERS_FALLBACK_MODEL
                    print(
                        f"⚠️  Transformers in this environment does not support '{text_model_name}'. "
                        f"Falling back to '{fallback_model}'."
                    )
                    resolved_text_model_name = fallback_model
                    self.text_model = AutoModelForCausalLM.from_pretrained(
                        resolved_text_model_name,
                        **text_model_kwargs
                    )
        ...
        try:
            ...
            error_text = str(e)
            # ESM3 checkpoints may require gated access. Fall back to open ESM-C model.
            if ("Cannot access gated repo" in error_text or "GatedRepoError" in error_text) and "esm3" in resolved_protein_model_name.lower():
                fallback_protein_model = ESM3_GATED_FALLBACK_MODEL
                print(
                    f"⚠️  Could not access gated protein model '{resolved_protein_model_name}'. "
                    f"Falling back to '{fallback_protein_model}'."
                )
                self.protein_encoder = create_protein_encoder(
                    fallback_protein_model,
                    inference_mode=not protein_model_finetune,
                    embedding_layer=-1,
                )
                resolved_protein_model_name = fallback_protein_model
            else:
                raise
        ...
        **kwargs,
        ...
        if protein_sequences is not None and batch_idx_map is not None:
            ...
            if n_protein_tokens > 0:
                # Keep dtype/device aligned with text embeddings to avoid FlashAttention dtype issues.
                protein_embeds_flat = protein_embeds_flat.to(
                    dtype=text_inputs_embeds.dtype,
