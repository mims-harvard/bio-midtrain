...
def qwen_protein_collate_fn(
    ...
    if inference_mode:
        ...
        # Expose flat protein_sequences + batch_idx_map for model.generate(...)
        flat_seqs: List[str] = []
        idx_map: List[int] = []
        for i, seqs in enumerate(batch_protein_sequences):
            for s in seqs:
                flat_seqs.append(s)
                idx_map.append(i)
        batch["protein_sequences"] = flat_seqs
        batch["batch_idx_map"] = idx_map
        batch["go_aspects"] = list(batch_go_aspects)
