"""
Convert GRPO-related checkpoints into a Hugging Face directory suitable for BioReason-Pro.

Supports:

1. **Full ProteinLLMModel** ``pytorch_model.bin`` (keys ``text_model.*``) — original path.
2. **train_protein_grpo.py** output: a Hugging Face folder with ``adapter_config.json`` (PEFT)
   or a merged causal LM (``config.json`` + ``*.safetensors`` / ``pytorch_model.bin`` with
   ``model.*`` keys). Use the same ``--text_model_name`` architecture as GRPO's base model;
   special-token rows in ``ProteinLLMModel`` stay initialized after the shared vocab prefix
   is copied from the GRPO checkpoint.
"""
...
from bioreason2.utils.grpo_checkpoint_convert import (
    GrpoCheckpointKind,
    detect_grpo_checkpoint_kind,
    inject_merged_causal_into_text_model,
    load_merged_causal_lm,
)
...
def save_grpo_ckpt(args):
    ...
    if args.checkpoint_format == "auto":
        ckpt_kind = detect_grpo_checkpoint_kind(args.checkpoint_path)
    else:
        ckpt_kind = GrpoCheckpointKind(args.checkpoint_format)

    print(f"📂 Checkpoint layout: {ckpt_kind.value}")
    ...
    if ckpt_kind == GrpoCheckpointKind.PROTEIN_LLM_PYTORCH_BIN:
        ...
        if checkpoint_vocab_size:
            ...
            if current_vocab_size != checkpoint_vocab_size:
                print(
                    f"⚠️  Vocab size mismatch: checkpoint={checkpoint_vocab_size}, model={current_vocab_size}"
                )
        ...
        if checkpoint_vocab_size:
            if hasattr(model.text_model, "base_model") and hasattr(
                model.text_model.base_model, "model"
            ):
            ...
            if (
                current_embed_size != checkpoint_vocab_size
                or current_lm_head_size != checkpoint_vocab_size
            ):
                ...
                if hasattr(model.text_model, "base_model") and hasattr(
                    ...
                    model.text_model.base_model.model.resize_token_embeddings(
        ...
        print("📊 Loading train_protein_grpo HF causal checkpoint (merge PEFT if needed)...")
        merged = load_merged_causal_lm(
        ...
        merged.eval()
        with torch.no_grad():
            inject_merged_causal_into_text_model(model.text_model, merged)
        del merged
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

        result = SimpleNamespace(missing_keys=(), unexpected_keys=())
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Directory with a checkpoint: full ProteinLLM ``pytorch_model.bin``, or "
            "``train_protein_grpo`` output (PEFT adapter dir or merged HF causal LM)."
        ),
    )
    parser.add_argument(
        "--checkpoint_format",
        ...
        choices=(
            "auto",
            GrpoCheckpointKind.PROTEIN_LLM_PYTORCH_BIN.value,
            GrpoCheckpointKind.HF_PEFT_ADAPTER.value,
            GrpoCheckpointKind.HF_MERGED_CAUSAL_LM.value,
        ),
        help="Override automatic checkpoint layout detection.",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
