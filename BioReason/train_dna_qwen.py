...
# Evo2/Vortex + TransformerEngine: avoid torch.compile inside TE jit helpers; Dynamo is also
# disabled so TE's @no_torch_dynamo wrappers don't interact badly with PL/DeepSpeed.
os.environ.setdefault("TORCHDYNAMO_DISABLE", "1")
os.environ.setdefault("NVTE_TORCH_COMPILE", "0")
os.environ.setdefault("BIOREASON_USE_VORTEX_PYTORCH_LINEAR", "1")

from bioreason.utils.force_vortex_pytorch_linear import install_import_hook

install_import_hook()

import torch

# Bug I workaround: legacy pickle save instead of zipfile.
# DeepSpeed's TorchCheckpointEngine + Lustre (holylfs06) hits inline_container.cc:664
# "unexpected pos N vs M" in zip writer's position tracking. Default to non-zip
# serialization for all torch.save calls in this process.
_orig_torch_save = torch.save
def _patched_torch_save(*args, **kwargs):
    kwargs.setdefault("_use_new_zipfile_serialization", False)
    return _orig_torch_save(*args, **kwargs)
...
class DNALLMFineTuner(pl.LightningModule):
    ...
    def _normalize_label_text(text: str) -> str:
        """Normalize generated/ground-truth text for robust label matching."""
        if text is None:
            return ""
    ...
    def _extract_predicted_label(self, generation: str) -> Tuple[Optional[str], str]:
        """
        Extract predicted label from a generated sequence.
        Returns (matched_label_from_self_labels_or_none, extracted_answer_text).
        """
        full_text = generation or ""
        answer_segment = full_text

        if "<|im_start|>assistant" in answer_segment:
            answer_segment = answer_segment.rsplit("<|im_start|>assistant", 1)[-1]
        if "Answer:" in answer_segment:
            answer_segment = answer_segment.rsplit("Answer:", 1)[-1]
        if "<|im_end|>" in answer_segment:
            answer_segment = answer_segment.split("<|im_end|>", 1)[0]

        answer_segment = answer_segment.strip()
        normalized_answer = self._normalize_label_text(answer_segment)
        normalized_full = self._normalize_label_text(full_text)

        normalized_labels: List[Tuple[str, str]] = []
        for label in self.labels:
            norm_label = self._normalize_label_text(label)
            if norm_label:
                normalized_labels.append((norm_label, label))
        normalized_labels.sort(key=lambda x: len(x[0]), reverse=True)

        for norm_label, raw_label in normalized_labels:
            if norm_label in normalized_answer:
                return raw_label, answer_segment

        for norm_label, raw_label in normalized_labels:
            if norm_label in normalized_full:
                return raw_label, answer_segment

        return None, answer_segment
    ...
    def _step(self, batch: Dict, batch_idx: int, prefix: str) -> torch.Tensor:
        ...
        if labels is not None and not torch.any(labels != -100):
            zero_loss = self.dna_projection.weight.sum() * 0.0
            self.log(
                f"{prefix}_empty_label_batch",
                1.0,
                on_step=True,
                on_epoch=True,
                prog_bar=False,
                logger=True,
                sync_dist=(prefix != "test"),
                batch_size=batch_size,
            )
            print(f"Warning: skipping {prefix} batch {batch_idx} because it contains no supervised target tokens.")
            return zero_loss
    ...
    def _load_kegg_dataset(self):
        local_root = os.path.abspath(os.path.expanduser(self.hparams.kegg_data_dir_local))
        ...
        return load_dataset(self.hparams.kegg_data_dir_huggingface)
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        if self.hparams.dataset_type == "kegg":
            dataset = self._load_kegg_dataset()
        ...
        frac = float(getattr(self.hparams, "train_data_fraction", 1.0) or 1.0)
        if 0.0 < frac < 1.0:
            n_full = len(train_dataset)
            n_keep = max(1, int(n_full * frac))
            train_dataset = train_dataset.select(range(n_keep))
            print(f"[train_data_fraction] keeping {n_keep}/{n_full} train examples (fraction={frac})")
    ...
    def val_dataloader(self) -> DataLoader:
        ...
        if self.hparams.dataset_type == "kegg":
            ...
            elif self.hparams.merge_val_test_set:
    ...
    def on_test_epoch_end(self):
        ...
        """
        if not self.return_answer_in_batch:
            raise ValueError(
                "on_test_epoch_end requires --return_answer_in_batch True so ground-truth answers are available."
            )
        ...
        pos_label = labels[1] if is_binary else None
        neg_label = labels[0] if is_binary else None
        ...
        if is_binary:
            ...
            print(f"Using binary labels - Positive: '{pos_label}', Negative: '{neg_label}'")
        else:
            print(f"Detected multiclass task with {len(labels)} labels.")
        wandb_logger.log(label_info)
        ...
        for batch_idx, batch in enumerate(test_dataloader):
            ...
            for example_idx in range(input_ids.size(0)):
                ...
                if total_examples > 0 and total_examples % 10 == 0:
                    current_accuracy = total_correct / max(1, total_examples)
                    wandb_logger.log({
                ...
                if assistant_pos is not None:
                    ...
                    predicted_label, predicted_answer_text = self._extract_predicted_label(generation)
                    is_exact_match = (
                        predicted_label is not None
                        and self._normalize_label_text(predicted_label) == self._normalize_label_text(ground_truth)
                    )
                    
                    # Update metrics
                    total_examples += 1
                    examples_in_batch += 1
                    total_correct += int(is_exact_match)
                    if predicted_label is None:
                        unmapped_predictions += 1
                    
                    prediction_category = "CORRECT" if is_exact_match else "WRONG"
                    is_positive_example = None

                    if is_binary:
                        is_positive_example = self._normalize_label_text(ground_truth) == self._normalize_label_text(pos_label)
                        predicted_is_positive = predicted_label is not None and (
                            self._normalize_label_text(predicted_label) == self._normalize_label_text(pos_label)
                        )
                        predicted_is_negative = predicted_label is not None and (
                            self._normalize_label_text(predicted_label) == self._normalize_label_text(neg_label)
                        )

                        if is_positive_example and predicted_is_positive:
                            true_positives += 1
                            prediction_category = "TP"
                        elif is_positive_example and not predicted_is_positive:
                            false_negatives += 1
                            prediction_category = "FN"
                        elif (not is_positive_example) and predicted_is_negative:
                            true_negatives += 1
                    ...
                    generations.append({
                        ...
                        "predicted_label": predicted_label if predicted_label is not None else "",
            ...
            current_accuracy = total_correct / max(total_examples, 1)
            progress_log = {
                "batches_processed": processed_batches,
                "examples_processed": total_examples,
                "examples_in_last_batch": examples_in_batch,
                "current_accuracy": current_accuracy,
                "unmapped_predictions": unmapped_predictions,
        ...
        final_metrics = {
            ...
            "test_total_correct": total_correct,
            "test_unmapped_predictions": unmapped_predictions,
        ...
        wandb_logger.log(final_metrics)
        ...
        if is_binary:
            summary = (
                ...
                f"Unmapped predictions: {unmapped_predictions}"
            )
        else:
            summary = (
                f"Test Results Summary:\n"
                f"Task: Multiclass ({len(labels)} labels)\n"
                f"Total examples: {total_examples}\n"
                f"Accuracy: {accuracy:.4f}\n"
...
def main(args: ArgumentParser):
    ...
    """
    args.cache_dir = _configure_non_home_cache_dirs(args.cache_dir)
    ...
    if args.eval_only or not args.skip_test:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_data_fraction", type=float, default=1.0,
                        help="Fraction of the train split to keep (0,1]. Selected deterministically from the head after dataset load.")
    ...
    parser.add_argument("--eval_split", type=str, choices=["", "id", "ood"], default="",
                        help="Restrict val/test_dataloader to ID (id_test_network_split.csv) or OOD (ood_test_network_split.csv) only. Overrides --merge_val_test_set.")
    parser.add_argument("--eval_only", type=bool, default=False,
                        help="Skip trainer.fit; just run trainer.test from --ckpt_path. Pair with --eval_split id|ood.")

    # Logging and monitoring
    parser.add_argument("--wandb_project", type=str, default="BioReason") #"nt-500m-qwen3-1.7b-finetune")
    parser.add_argument("--wandb_entity", type=str, default="harvardml")
