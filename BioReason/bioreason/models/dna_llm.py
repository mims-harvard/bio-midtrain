...
class DNALLMModel(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if dna_tokenized is not None and batch_idx_map:
            ...
            text_inputs_embeds = self._inject_dna_embeddings(text_inputs_embeds, mask, dna_embeds_flat)
    ...
    def is_gradient_checkpointing(self) -> bool:
        return bool(getattr(self.text_model, "is_gradient_checkpointing", False))

    def gradient_checkpointing_disable(self):
        if hasattr(self.text_model, "gradient_checkpointing_disable"):
            self.text_model.gradient_checkpointing_disable()
        print("gradient_checkpointing_disable for model:", getattr(self.text_model, "is_gradient_checkpointing", False))
    ...
    def _inject_dna_embeddings(
        self,
        text_inputs_embeds: torch.Tensor,
        mask: torch.Tensor,
        dna_embeds_flat: torch.Tensor,
    ) -> torch.Tensor:
        """
        Place DNA embeddings into `<|dna_pad|>` slots.
        If sequence truncation causes slot-count and embedding-count mismatch,
        align using the tail (left-truncation keeps the right side).
        """
        num_slots = int(mask.sum().item())
        num_embeds = int(dna_embeds_flat.shape[0])

        if num_slots == 0 or num_embeds == 0:
            return text_inputs_embeds

        if num_embeds == num_slots:
            text_inputs_embeds[mask] = dna_embeds_flat
            return text_inputs_embeds

        # Align by suffix to match left-truncated prompts.
        if num_embeds > num_slots:
            dna_embeds_flat = dna_embeds_flat[-num_slots:]
            text_inputs_embeds[mask] = dna_embeds_flat
            return text_inputs_embeds

        # num_embeds < num_slots: only fill the last num_embeds slots.
        flat_embeds = text_inputs_embeds.reshape(-1, text_inputs_embeds.size(-1))
        mask_flat = mask.reshape(-1)
        slot_indices = torch.nonzero(mask_flat, as_tuple=False).squeeze(-1)
        target_indices = slot_indices[-num_embeds:]
        flat_embeds[target_indices] = dna_embeds_flat
        return flat_embeds.reshape_as(text_inputs_embeds)
