...
class DLProcessorKwargs(ProcessingKwargs, total=False):
    ...
    Do not add custom ``*_kwargs`` keys (e.g. ``dna_kwargs``) here: HuggingFace
    ``ProcessorMixin._merge_kwargs`` only builds ``text`` / ``images`` / ``audio``
    / ``videos`` buckets and will KeyError on any extra annotated modality.
    DNA lengths are passed as explicit ``__call__`` arguments instead.
    """
